...
install_rust() {
    if command -v rustc >/dev/null 2>&1; then
        ...
        curl --proto '=https' --tlsv1.2 -sSf -k https://sh.rustup.rs | sh -s -- -y
...
install_solana_cli() {
    local os="$1"
    local install_cmd='sh -c "$(curl -sSfL -k https://release.anza.xyz/stable/install)"'
...
install_nvm_and_node() {
    if [ -s "$HOME/.nvm/nvm.sh" ]; then
        ...
        curl -o- -k https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.3/install.sh | bash
    fi
